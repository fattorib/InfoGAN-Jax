...
class TestUtils(unittest.TestCase):
    ...
    def test_create_latents(self):
        ...
        z = create_latents_with_codes(num_noise=62, num_cts=2, num_cat=10, rng_key = self.rng, num_samples=128)
        self.assertEqual((128,74), z.shape)


        z = create_latents_with_codes(num_noise=62, num_cts=2, num_cat=11, rng_key = self.rng, num_samples=1)
...
class TestGenerator(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.variables = self.model.init(self.rng, jnp.ones(input_shape, dtype=jnp.float32), train=True)
        
        self.z = create_latents_with_codes(num_noise=62, num_cts=2, num_cat=10, rng_key = self.rng, num_samples=128)


    def test_generator_apply(self):

        output, _ = self.model.apply(
            {"params": self.variables["params"], "batch_stats": self.variables["batch_stats"]},
            self.z,
            mutable=["batch_stats"],
            train=False)
